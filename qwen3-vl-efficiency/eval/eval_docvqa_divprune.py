"""
eval_docvqa_divprune.py — DocVQA evaluation with DivPrune visual token pruning.

Usage:
    python -m eval.eval_docvqa_divprune --subset_ratio 0.5 --max_samples 100
    python -m eval.eval_docvqa_divprune --no_divprune --max_samples 100
"""
...
DATASET_HF = "HuggingFaceM4/DocumentVQA"
SPLIT = "validation"
MAX_NEW_TOKENS = 32
...
def parse_args():
    parser = argparse.ArgumentParser(description="DocVQA evaluation with DivPrune.")
...
def run_ratio(model, processor, dataset, ratio, max_samples, verbose, results_dir):
    ...
    results_path = f"{results_dir}/docvqa_{tag}.jsonl"

    print("=" * 80)
    print(f"DocVQA Evaluation — DivPrune  |  ratio={'baseline' if ratio is None else ratio}")
    print("=" * 80)
    ...
    with open(results_path, "w") as f:
        for idx in range(min(limit, len(dataset))):
            ...
            ground_truth = sample.get("answers", ["N/A"])
            ...
            gt_display = ground_truth if isinstance(ground_truth, str) else " | ".join(ground_truth[:3])
            row = {
                "sample_id": idx,
                "dataset": "docvqa",
                "subset_ratio": ratio,
            ...
            print(
                f"[{idx:>4}] GT={str(gt_display)[:20]:<20}  Pred={prediction[:20]:<20}  "
...
def main():
    ...
    dataset = load_dataset(DATASET_HF, split=SPLIT, verification_mode="no_checks")
