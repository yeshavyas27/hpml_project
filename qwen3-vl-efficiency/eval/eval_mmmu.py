...
RESULTS_PATH = "results/baseline/mmmu_results.jsonl"
MAX_SAMPLES = 2
...
def main():
    ...
    with open(RESULTS_PATH, "w") as f:
        for idx in range(min(MAX_SAMPLES, len(dataset))):
            ...
            full_question = question + "\nOptions:\n" + "\n".join(options) + "\nPlease answer with only the correct option letter and nothing else."
            
            reset_gpu_memory()
            prediction, latency_sec = timed_inference(
                generate_answer, model, processor, image, full_question
            )
